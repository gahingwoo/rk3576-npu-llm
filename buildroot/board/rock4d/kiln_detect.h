...
// kiln_detect.h -- object detection for the RK3576 NPU via librknnrt (RKNN).
// YOLOv8/YOLO11 (anchor-free DFL), YOLOv5/v7 (anchor-based), YOLOX (anchor-free +
// objectness), and YOLO-RAW: a decoded pre-NMS single output [1,N,4+ncls]
// (Ultralytics nms=False, e.g. YOLO26/YOLOv10) -- the format that runs on the RK3576
// NPU (pure conv; CPU does NMS). (An END2END [1,N,6] NMS-in-model export also has a
// decoder but CRASHES on the NPU -- the NMS ops aren't supported.)
//
// Verified: YOLOv8n runs end-to-end on a ROCK 4D (bicycle/truck/dog on the classic
// dog-bike-car image, ~37 ms). Host unit tests (no NPU) also cover the letterbox
// preprocessing + inverse box mapping, IoU, per-class NMS, box+label drawing, and every
// per-branch decoder (planted synthetic tensors -> expected boxes). A DIFFERENT family
// or export (v5/v7 anchors, YOLOX, a yolo-raw export) is tested on fewer real models --
// treat a new one as "confirm the printed family + first boxes once". Detection is OFF
// by default (`[vision] task = classify`).
//
...
public:
    ...
    // Draw box outlines (per-class colour) into an RGB buffer (w*h*3), each with a
    // "<label> <score>" bar drawn in the class colour above the box (or just inside the
    // top edge if there's no room above). Text colour flips black/white for contrast.
    static void draw_boxes(unsigned char *rgb, int w, int h, const std::vector<KilnDetection> &dets, int thick = 3) {
        int scale = w >= 1280 ? 2 : 1;   // larger text on large images
        for (const auto &d : dets) {
            ...
            char sc[16]; snprintf(sc, sizeof(sc), " %.2f", d.score);
            std::string lab = (d.label.empty() ? std::to_string(d.class_id) : d.label) + sc;
            unsigned luma = (299u * r + 587u * g + 114u * b) / 1000u;   // ITU-R 601
            unsigned char t = luma > 140 ? 0 : 255;                    // dark text on light bars
            int bar_h = 13 * scale + 2;
            int ly = (int)d.box.y1 - bar_h;
            if (ly < 0) ly = (int)d.box.y1 + thick;                    // no room above -> inside the box
            blit_label(rgb, w, h, (int)d.box.x1, ly, lab, r, g, b, t, t, t, scale);
        }
    }
...
private:
    ...
    // Draw a short ASCII label with a filled background bar, using a bundled 8x13
    // bitmap font (below). scale>1 blits each font pixel as a scale*scale block, so
    // text stays readable on large images. The origin is clamped to keep the bar
    // on-screen. Non-printable chars render as '?'.
    static void blit_label(unsigned char *rgb, int W, int H, int px0, int py0, const std::string &s,
                           unsigned char br, unsigned char bg, unsigned char bb,     // bar (class) colour
                           unsigned char fr, unsigned char fg, unsigned char fb,     // text colour
                           int scale) {
        // 8x13 1bpp font, printable ASCII 0x20-0x7E (95 glyphs), 1 byte/row, MSB=left.
        // Generated once from DejaVu Sans Mono (public domain, Bitstream Vera derivative).
        static const unsigned char F[95][13] = {
            {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00}, // space
            {0x00,0x00,0x00,0x18,0x18,0x18,0x18,0x18,0x18,0x00,0x18,0x18,0x00}, // !
            {0x00,0x00,0x00,0x24,0x24,0x24,0x24,0x00,0x00,0x00,0x00,0x00,0x00}, // "
            {0x00,0x00,0x12,0x12,0x14,0x7f,0x24,0x24,0xfe,0x68,0x48,0x48,0x00}, // #
            {0x00,0x00,0x00,0x00,0x3c,0x68,0x40,0x38,0x1c,0x02,0x46,0x3c,0x00}, // $
            {0x00,0x00,0x00,0x70,0x90,0x90,0x76,0x18,0x6e,0x0b,0x0b,0x0e,0x00}, // %
            {0x00,0x00,0x00,0x3c,0x60,0x20,0x30,0x59,0xcb,0xc6,0x46,0x3a,0x00}, // &
            {0x00,0x00,0x00,0x18,0x18,0x18,0x18,0x00,0x00,0x00,0x00,0x00,0x00}, // '
            {0x00,0x08,0x08,0x18,0x10,0x10,0x10,0x10,0x10,0x10,0x18,0x08,0x08}, // (
            {0x00,0x30,0x10,0x18,0x08,0x08,0x08,0x08,0x08,0x08,0x18,0x10,0x30}, // )
            {0x00,0x00,0x00,0x10,0x52,0x38,0x38,0x52,0x10,0x00,0x00,0x00,0x00}, // *
            {0x00,0x00,0x00,0x00,0x18,0x18,0x18,0xfe,0x18,0x18,0x18,0x00,0x00}, // +
            ...
            {0x00,0x00,0x00,0x06,0x04,0x0c,0x08,0x08,0x10,0x10,0x30,0x20,0x60}, // /
            {0x00,0x00,0x00,0x3c,0x64,0x46,0x42,0x5a,0x42,0x46,0x64,0x3c,0x00}, // 0
            {0x00,0x00,0x00,0x78,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x3e,0x00}, // 1
            {0x00,0x00,0x00,0x3c,0x44,0x06,0x04,0x0c,0x18,0x30,0x60,0x7e,0x00}, // 2
            {0x00,0x00,0x00,0x3c,0x44,0x06,0x04,0x3c,0x06,0x06,0x46,0x3c,0x00}, // 3
            {0x00,0x00,0x00,0x0c,0x1c,0x14,0x24,0x64,0x44,0x7e,0x04,0x04,0x00}, // 4
            {0x00,0x00,0x00,0x7c,0x60,0x60,0x7c,0x04,0x06,0x06,0x44,0x38,0x00}, // 5
            {0x00,0x00,0x00,0x3c,0x60,0x40,0x7c,0x66,0x42,0x42,0x66,0x3c,0x00}, // 6
            {0x00,0x00,0x00,0x7e,0x06,0x04,0x0c,0x08,0x18,0x18,0x10,0x30,0x00}, // 7
            {0x00,0x00,0x00,0x3c,0x66,0x46,0x64,0x3c,0x66,0x42,0x66,0x3c,0x00}, // 8
            {0x00,0x00,0x00,0x3c,0x64,0x46,0x46,0x66,0x3e,0x06,0x04,0x38,0x00}, // 9
            ...
            {0x00,0x00,0x00,0x00,0x00,0x02,0x1c,0x60,0x60,0x1c,0x02,0x00,0x00}, // <
            {0x00,0x00,0x00,0x00,0x00,0x00,0xfe,0x00,0x00,0xfe,0x00,0x00,0x00}, // =
            {0x00,0x00,0x00,0x00,0x00,0x40,0x38,0x0e,0x0e,0x38,0x40,0x00,0x00}, // >
            {0x00,0x00,0x00,0x3c,0x06,0x06,0x0c,0x18,0x10,0x00,0x10,0x10,0x00}, // ?
            {0x00,0x00,0x00,0x3c,0x62,0x42,0xcf,0x93,0x93,0x93,0xcf,0x40,0x60}, // @
            {0x00,0x00,0x00,0x18,0x18,0x3c,0x2c,0x24,0x66,0x7e,0x42,0xc2,0x00}, // A
            {0x00,0x00,0x00,0x7c,0x46,0x46,0x46,0x7c,0x46,0x42,0x46,0x7c,0x00}, // B
            {0x00,0x00,0x00,0x1c,0x22,0x60,0x40,0x40,0x40,0x60,0x22,0x1c,0x00}, // C
            {0x00,0x00,0x00,0x78,0x44,0x46,0x42,0x42,0x42,0x46,0x44,0x78,0x00}, // D
            {0x00,0x00,0x00,0x7e,0x60,0x60,0x60,0x7e,0x60,0x60,0x60,0x7e,0x00}, // E
            {0x00,0x00,0x00,0x7e,0x60,0x60,0x60,0x7e,0x60,0x60,0x60,0x60,0x00}, // F
            {0x00,0x00,0x00,0x3c,0x62,0x40,0x40,0x4e,0x42,0x42,0x62,0x3c,0x00}, // G
            {0x00,0x00,0x00,0x42,0x42,0x42,0x42,0x7e,0x42,0x42,0x42,0x42,0x00}, // H
            {0x00,0x00,0x00,0x7e,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x7e,0x00}, // I
            {0x00,0x00,0x00,0x3c,0x04,0x04,0x04,0x04,0x04,0x04,0x4c,0x78,0x00}, // J
            {0x00,0x00,0x00,0x42,0x44,0x48,0x70,0x78,0x48,0x4c,0x46,0x42,0x00}, // K
            {0x00,0x00,0x00,0x60,0x60,0x60,0x60,0x60,0x60,0x60,0x60,0x7e,0x00}, // L
            {0x00,0x00,0x00,0xe6,0xe6,0xe6,0xfa,0xda,0xda,0xc2,0xc2,0xc2,0x00}, // M
            {0x00,0x00,0x00,0x62,0x62,0x72,0x52,0x5a,0x4a,0x4e,0x46,0x46,0x00}, // N
            {0x00,0x00,0x00,0x3c,0x66,0x46,0x42,0x42,0x42,0x46,0x66,0x3c,0x00}, // O
            {0x00,0x00,0x00,0x7c,0x66,0x62,0x62,0x66,0x7c,0x60,0x60,0x60,0x00}, // P
            {0x00,0x00,0x00,0x3c,0x66,0x46,0x42,0x42,0x42,0x46,0x66,0x3c,0x0c}, // Q
            {0x00,0x00,0x00,0x7c,0x46,0x46,0x46,0x7c,0x4c,0x46,0x42,0x43,0x00}, // R
            {0x00,0x00,0x00,0x3c,0x60,0x40,0x60,0x3c,0x06,0x02,0x46,0x3c,0x00}, // S
            {0x00,0x00,0x00,0xff,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x00}, // T
            {0x00,0x00,0x00,0x42,0x42,0x42,0x42,0x42,0x42,0x42,0x66,0x3c,0x00}, // U
            {0x00,0x00,0x00,0xc2,0x42,0x46,0x64,0x24,0x24,0x3c,0x18,0x18,0x00}, // V
            {0x00,0x00,0x00,0x83,0x83,0xc2,0xda,0x5a,0x5a,0x6e,0x66,0x66,0x00}, // W
            {0x00,0x00,0x00,0x42,0x66,0x3c,0x18,0x18,0x3c,0x24,0x66,0xc2,0x00}, // X
            {0x00,0x00,0x00,0xc2,0x66,0x24,0x3c,0x18,0x18,0x18,0x18,0x18,0x00}, // Y
            {0x00,0x00,0x00,0x7e,0x06,0x04,0x0c,0x18,0x10,0x20,0x60,0x7e,0x00}, // Z
            {0x00,0x1c,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x1c}, // [
            {0x00,0x00,0x00,0x40,0x60,0x20,0x30,0x10,0x10,0x08,0x08,0x0c,0x04}, // backslash
            {0x00,0x38,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x08,0x38}, // ]
            {0x00,0x00,0x00,0x18,0x3c,0x64,0x42,0x00,0x00,0x00,0x00,0x00,0x00}, // ^
            {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00}, // _
            {0x00,0x00,0x30,0x10,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00}, // `
            {0x00,0x00,0x00,0x00,0x00,0x3c,0x44,0x06,0x3e,0x46,0x46,0x3e,0x00}, // a
            {0x00,0x40,0x40,0x40,0x40,0x7c,0x66,0x62,0x42,0x62,0x66,0x7c,0x00}, // b
            {0x00,0x00,0x00,0x00,0x00,0x1c,0x22,0x60,0x60,0x60,0x22,0x1c,0x00}, // c
            {0x00,0x06,0x06,0x06,0x06,0x3e,0x66,0x46,0x46,0x46,0x66,0x3e,0x00}, // d
            {0x00,0x00,0x00,0x00,0x00,0x3c,0x66,0x42,0x7e,0x40,0x62,0x3c,0x00}, // e
            {0x00,0x0e,0x18,0x10,0x10,0x7e,0x10,0x10,0x10,0x10,0x10,0x10,0x00}, // f
            {0x00,0x00,0x00,0x00,0x00,0x3e,0x66,0x46,0x46,0x46,0x66,0x3e,0x06}, // g
            {0x00,0x40,0x40,0x40,0x40,0x7c,0x66,0x66,0x46,0x46,0x46,0x46,0x00}, // h
            {0x00,0x18,0x00,0x00,0x00,0x38,0x18,0x18,0x18,0x18,0x18,0x7e,0x00}, // i
            {0x00,0x08,0x00,0x00,0x00,0x38,0x08,0x08,0x08,0x08,0x08,0x08,0x08}, // j
            {0x00,0x60,0x60,0x60,0x60,0x66,0x6c,0x78,0x78,0x6c,0x66,0x62,0x00}, // k
            {0x00,0x70,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x10,0x0e,0x00}, // l
            {0x00,0x00,0x00,0x00,0x00,0x7e,0x5a,0x5a,0x5a,0x5a,0x5a,0x5a,0x00}, // m
            {0x00,0x00,0x00,0x00,0x00,0x7c,0x66,0x66,0x46,0x46,0x46,0x46,0x00}, // n
            {0x00,0x00,0x00,0x00,0x00,0x3c,0x66,0x42,0x42,0x42,0x66,0x3c,0x00}, // o
            {0x00,0x00,0x00,0x00,0x00,0x7c,0x66,0x62,0x42,0x62,0x66,0x7c,0x40}, // p
            {0x00,0x00,0x00,0x00,0x00,0x3e,0x66,0x46,0x46,0x46,0x66,0x3e,0x06}, // q
            {0x00,0x00,0x00,0x00,0x00,0x3e,0x30,0x30,0x30,0x30,0x30,0x30,0x00}, // r
            {0x00,0x00,0x00,0x00,0x00,0x3c,0x64,0x60,0x3c,0x04,0x44,0x3c,0x00}, // s
            {0x00,0x00,0x00,0x10,0x10,0x7e,0x10,0x10,0x10,0x10,0x10,0x1e,0x00}, // t
            {0x00,0x00,0x00,0x00,0x00,0x46,0x46,0x46,0x46,0x66,0x66,0x3e,0x00}, // u
            {0x00,0x00,0x00,0x00,0x00,0x42,0x46,0x64,0x24,0x2c,0x18,0x18,0x00}, // v
            {0x00,0x00,0x00,0x00,0x00,0x83,0xc3,0x5a,0x5a,0x7a,0x66,0x64,0x00}, // w
            {0x00,0x00,0x00,0x00,0x00,0x46,0x24,0x18,0x18,0x38,0x24,0x42,0x00}, // x
            {0x00,0x00,0x00,0x00,0x00,0x42,0x66,0x24,0x24,0x3c,0x18,0x18,0x18}, // y
            {0x00,0x00,0x00,0x00,0x00,0x7e,0x04,0x08,0x18,0x30,0x20,0x7e,0x00}, // z
            {0x00,0x0c,0x18,0x18,0x18,0x10,0x70,0x10,0x18,0x18,0x18,0x18,0x0c}, // {
            {0x00,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18,0x18}, // |
            {0x00,0x70,0x10,0x18,0x18,0x18,0x0c,0x18,0x18,0x18,0x10,0x10,0x70}, // }
            {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x70,0x0e,0x00,0x00,0x00,0x00}, // ~
        };
        const int GW = 8, GH = 13, pad = 1;
        if (scale < 1) scale = 1;
        int tw = (int)s.size() * GW * scale + 2 * pad;
        int th = GH * scale + 2 * pad;
        if (px0 < 0) px0 = 0;
        if (py0 < 0) py0 = 0;
        if (px0 + tw > W) px0 = std::max(0, W - tw);
        if (py0 + th > H) py0 = std::max(0, H - th);
        for (int y = 0; y < th; y++)
            for (int x = 0; x < tw; x++)
                px(rgb, W, H, px0 + x, py0 + y, br, bg, bb);   // background bar (class colour)
        int cx = px0 + pad, cy = py0 + pad;
        for (unsigned char ch : s) {
            if (ch < 0x20 || ch > 0x7e) ch = '?';
            const unsigned char *g = F[ch - 0x20];
            for (int gy = 0; gy < GH; gy++)
                for (int gx = 0; gx < GW; gx++)
                    if ((g[gy] >> (7 - gx)) & 1)
                        for (int sy = 0; sy < scale; sy++)
                            for (int sx = 0; sx < scale; sx++)
                                px(rgb, W, H, cx + gx * scale + sx, cy + gy * scale + sy, fr, fg, fb);
            cx += GW * scale;
        }
    }
